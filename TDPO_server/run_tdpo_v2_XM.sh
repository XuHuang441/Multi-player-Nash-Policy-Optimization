...
run_iteration() {
    ...
    # CUDA_VISIBLE_DEVICES=2 python ./generation/get_hf2.py --model_name_or_path ${previous_model} --dataset_name_or_path ${input_path} --output_dir ${json_output} --sanity_check $sanity_check --K $K --temperature 1.0 --local_index 0 --my_world_size ${my_world_size} --eos_ids 128009 &
    # CUDA_VISIBLE_DEVICES=3 python ./generation/get_hf2.py --model_name_or_path ${previous_model} --dataset_name_or_path ${input_path} --output_dir ${json_output} --sanity_check $sanity_check --K $K --temperature 1.0 --local_index 1 --my_world_size ${my_world_size} --eos_ids 128009 &
    for idx in $(seq 0 $((my_world_size-1))); do
        CUDA_VISIBLE_DEVICES=$idx python ./generation/get_hf2.py \
    ...
    for idx in $(seq 0 $((my_world_size-1))); do
        CUDA_VISIBLE_DEVICES=$idx accelerate launch annotate_data/get_pref_single.py \
        --use_tournament $use_tour \
        --dataset_name_or_path "${json_output}_${idx}.json" \
        --output_dir           "${pref_output}_${idx}.json" \
        --K $K &
    done
    wait
